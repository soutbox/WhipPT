...
public class WhipPTService {
    ...
    public Flux<OllamaResponse> getChatStream(String prompt) {
        ...
        return ollamaWebClient.post().uri("/api/generate").bodyValue(request).retrieve().bodyToFlux(OllamaResponse.class).onErrorResume(e -> {
    ...
    /* PC의 Ollama 서버가 켜져 있는지 확인하는 초경량 헬스체크 메서드. 3초 안에 응답이 없으면 PC가 꺼진 것으로 간주(false) */
    public Mono<Boolean> checkOllamaHealth() {
        return ollamaWebClient.get().uri("/").retrieve().toBodilessEntity().map(response -> true).timeout(Duration.ofSeconds(3)).onErrorReturn(false);
    }
}